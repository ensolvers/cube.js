...
data. However, there are some rules that apply to when Cube.js uses the
...
`2021-01-20` in the the sample data:
...
`profit_margin` matches the above:

```
# Using `cost + (cost * profit_margin) = price` for row 1
x + (x * 1) = 275
2x          = 275
x           = 275 / 2
x           = 137.5

# Using `cost + (cost * profit_margin) = price` for row 2
x + (x * 0.1) = 248
1.1x          = 248
x             = 225.454545454545455
```

Now that we have the original `cost` for each item, let's use the same formula
to see if we get the same value (`523`) as calculated earlier:

```
# Using `cost + (cost * profit_margin) = price` to find `price` this time
362.954545454545455 + (362.954545454545455 * 1.1) = y
762.204545454545455                               = y
y                                                 = 762.204545454545455
```
